...
const TYPES_REQUIRING_RNC = ['E31', 'E33', 'E34', 'E41', 'E44', 'E45'];
...
class ReferenceDto {
  @ApiProperty({ description: 'eNCF del documento original', example: 'E310000000001' })
  @IsString()
  @Matches(/^E\d{12}$/, { message: 'eNCF inválido. Formato: E + 12 dígitos (ej: E310000000001)' })
  encf: string;
  ...
  @ApiProperty({ description: 'Código: 1=Anula NCF, 2=Corrige texto, 3=Corrige montos, 4=Reemplazo contingencia' })
  @IsInt({ message: 'Código de modificación debe ser un entero' })
  @IsIn([1, 2, 3, 4], { message: 'Código de modificación debe ser 1 (Anula), 2 (Corrige texto), 3 (Corrige montos) o 4 (Reemplazo contingencia)' })
  modificationCode: number;
}

class CurrencyDto {
  @ApiProperty({ description: 'Código ISO 4217 moneda (3 letras)', example: 'USD' })
  @IsString()
  @Matches(/^[A-Z]{3}$/, { message: 'Código de moneda debe ser 3 letras mayúsculas ISO 4217 (ej: USD, EUR)' })
  code: string;
...
export class CreateInvoiceDto {
  ...
  @ArrayMaxSize(1000, { message: 'Máximo 1000 items por factura' })
